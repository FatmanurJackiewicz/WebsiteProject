...
public class AuthAdminController : Controller
{
    ...
    public async Task<IActionResult> ForgotPassword([FromForm] ForgotPasswordViewModel forgotPasswordViewModel)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ViewBag.SuccessMessage = "Şifre sıfırlama e-postası gönderildi. Lütfen e-postanızı kontrol edin.";
            ModelState.Clear();
        }
        else
        {
    ...
    public async Task<IActionResult> RenewPassword([FromForm] RenewPasswordViewModel renewPasswordViewModel)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ViewBag.SuccessMessage = "Şifre başarıyla sıfırlandı. Giriş yapabilirsiniz.";
            return RedirectToAction(nameof(Login));
        }
        else
        {
            ModelState.AddModelError(string.Empty, "Şifre sıfırlama işlemi başarısız. Lütfen tekrar deneyiniz.");
